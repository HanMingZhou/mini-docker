...
### (b) 创建管道、计算 clone flags（54–66 行） container_linux.go:60-66
```go
...
### (c) 启动 init 子进程（68–72 行）container_linux.go:68-72
```go
...
### (f) 配置 cgroup（120–141 行）container_linux.go:136
```go 
...
## 3. wait() 和 release() container_linux.go:184-203
```go 
...
```
读取并解析父进程发来的 JSON 指令。container_linux.go:229-244
```go
...
# 子进程继承管道读端为 FD 3 是怎么做到的？
背景：FD 在 fork/exec 时的继承规则
Linux 的进程模型里有一条重要规则：子进程默认继承父进程所有打开的文件描述符（FD），除非该 FD 被标记了 O_CLOEXEC（"exec 时关闭"）。

但"继承"不等于"编号一样"。父进程里的某个 pipe 可能是 FD 7，到子进程里也是 7。问题是：子进程怎么知道要从 FD 几去读？ 总不能用环境变量传一个数字过去吧（虽然也行，但难看）。
...
container_linux.go:72
cmd.ExtraFiles = []*os.File{r}
子进程：

container_linux.go:214
pipe := os.NewFile(uintptr(initPipeFD), "init-pipe")
...
容器要做的事是：让容器进程看到的 / 变成另一个目录（比如 /var/lib/mydocker/containers/abc/merged，那里面解压了 alpine 镜像）。这样容器里 ls / 看到的就是 alpine 的内容，完全意识不到宿主机的存在。
...
这正是你代码里 ensureControllerAt 干的事： cgroup_linux.go:114-132
```go
...
Step 1：NewWithConfig —— 只算路径，不操作 cgroup_linux.go:40-44
```go
...
Step 2：Apply(resources) —— 创建 cgroup + 写限额 cgroup_linux.go:59-91
```go
...
Step 3：AddProc(pid) —— 把进程关进笼子 cgroup_linux.go:93-954
```go
...
Step 4：Destroy() —— 容器退出时清理 cgroup_linux.go:97-102
```go
func (m *v2Manager) Destroy() error {
if err := os.Remove(m.path); err != nil && !os.IsNotExist(err) {
 ```
rmdir /sys/fs/cgroup/mydocker/abc123。注意只有 cgroup 里没有进程时才能删（容器都退出了，所以可以删）。

## 五、为什么必须在父进程里 AddProc？ container_linux.go:136
```go
if err := cg.AddProc(cmd.Process.Pid); err != nil {
 ```
...
## 六、v1 vs v2
你代码里有这一行： cgroup_linux.go:19-21
```go
if !isV2() {
...
overlay 和 pivot_root 的关系：两件事，缺一不可
你的疑问非常自然——既然 merged 已经是一个"完整的镜像文件系统"了，为什么还要 pivot_root？
...
container_linux.go + rootfs_linux.go 的工作（在子进程里）
目标：把进程的根目录从宿主机 / 切换到 merged。
...
五、不懂的知识点请直接列出来
你说"有些知识点根本不懂"——这很正常，容器涉及的内核概念太多了。我建议你把不懂的具体词列出来，我一个一个给你讲。我先猜几个你可能卡住的点，看看是不是：

候选清单（看哪些你想深入）
下面这些哪些是你想我详细讲的？（可多选）

Skipped
用户跳过了问题。我就把回答停在这里，等用户提出具体不懂的点。

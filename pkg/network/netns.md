...
2.3 bridge 模式（最复杂，也是默认）
容器有自己的 netns，但通过一对虚拟网线连接到一个虚拟交换机，这个交换机再跟宿主机网络打通。
...
宿主机查路由表，发现走 eth0（真实物理网卡）出去
关键：源 IP 是 10.22.0.2，运营商不认这个内网 IP——这时 iptables 的 MASQUERADE 规则把源 IP 改成宿主机的公网 IP（NAT）
包发出去，回包按反向路径进来
...
路由表查 10.22.0.2 → 走 mydocker0
bridge 转发到对应 veth → 容器 eth0
容器里的 nginx 监听在 80 → 接受请求
...
建 veth pair
一头进容器 netns（netnsPath），一头接到 bridge
容器内 eth0 配 IP（IPAM 子插件分配，如 host-local）
加默认路由
调用 CNI 插件 portmap（如果有 ports）下 iptables DNAT 规则
返回容器拿到的 IP（如 10.22.0.2）
...
一句话总结
容器网络 = 把"路由器 + 网线 + NAT"用 Linux 内核的 netns/bridge/veth/iptables 模拟出来。 --network bridge 给容器一个独立 netns 然后用一对虚拟网线接到虚拟交换机；--network host 不建新 netns；--network none 建空 netns 啥也没接。我们的代码不直接搞这些原语，而是通过 CNI 插件来执行——插件读配置、建 veth、配 IP、加 iptables 规则，最后返回容器 IP 给我们记录。
...
容器 netns 销毁时，里面的网卡（veth 的一端）跟着消失
veth 是成对绑定的，一端消失，另一端自动消失
所以不需要手动清理 host 端的 veth，CNI plugin 也不用做额外工作
...
容器 A（10.22.0.2）发出 ping 8.8.8.8：
...
解析 flag（--network、-p）
决定要不要创建新 netns（nsFlags.Network = false 表示 host）
...
│   1. clone() 出新进程，带 CLONE_NEWNET（创建新 netns）│
...
从配置的子网中找一个空闲 IP（如 10.22.0.5）
记到本地数据库 /var/lib/cni/networks/<network>/<ip>，避免被重复分配
容器删除时（CNI DEL）从数据库释放
portmap 插件（/opt/cni/bin/portmap）的工作（端口映射）
接收 CapabilityArgs 里的 portMappings: [{HostPort: 8080, ContainerPort: 80, ...}]
在 nat 表的 PREROUTING 链下 DNAT：
iptables -t nat -A CNI-DN-XXX -p tcp --dport 8080 -j DNAT --to-destination 10.22.0.5:80
在 OUTPUT 链下同样规则（让本机也能 curl 127.0.0.1:8080）
在 POSTROUTING 链下 hairpin 模式 MASQUERADE（让容器自己 curl 自己的映射端口能通）
...
install-cni.sh 28 - 91
这就是 CNI 配置——一个 plugin 链：
...
容器内部： 数据包从容器内的网卡（eth0）发出。

网线传输： 通过 veth pair 这根虚拟网线，瞬间传到宿主机上的对端网卡（veth_host）。

接入网桥： 因为 veth_host 已经被奴役（Enslave）给了网桥，所以包直接进入了网桥这个虚拟交换机。
